...
| 名词 | 含义 | 本项目例子 |
| --- | --- | --- |
| 软件包 package | 组织相关程序、配置和资源的单位 | `mycontrol`、`mynav` |
...
| [src/mynav](src/mynav) | `mynav` | 点云转扫描、SLAM、Nav2 和探索的配置与启动 |
...
[scan.yaml](src/mynav/config/scan.yaml) 当前主要设置：
...
[slam_toolbox.yaml](src/mynav/config/slam_toolbox.yaml) 当前 `mode: mapping`。这是边建图边导航的流程，不要求先有一张完整地图。
...
[explore_params.yaml](src/mynav/config/explore_params.yaml) 当前设置：
...
my_bt/navigation.launch.py
    ↓ 读取 mynav 的 Nav2 配置，加入默认 BT XML 路径
mynav/navigation.launch.py
    ↓ 传递完整参数文件
...
```bash
# 扫描转换 + SLAM + Nav2 + 自动探索
ros2 launch mynav explore_bringup.launch.py
```
...
```bash
ros2 launch mynav explore.launch.py
```

**本模式不要再运行 `explore_bringup.launch.py` 或原来的 `mynav navigation.launch.py`。** `my_bt` 已经包含导航栈，重复启动会产生重复节点和接口。
...
[nav2_params.yaml](src/mynav/config/nav2_params.yaml) 中，bt_navigator 和 velocity_smoother 指定了 `/odin1/odometry`，controller_server 没有。Jazzy 控制器使用的 OdomSubscriber 默认话题为 `odom`，当前 launch 也没有为它重映射这个话题。
...
建议由上层 bringup 包组合现有包，避免让 mynav 和 my_bt 互相依赖形成环。统一入口需要复用已有功能，不必再复制一份完整 Nav2 参数。
...
| 点云高度筛选、二维扫描范围 | [scan.yaml](src/mynav/config/scan.yaml) |
| SLAM 分辨率、扫描匹配、地图更新 | [slam_toolbox.yaml](src/mynav/config/slam_toolbox.yaml) |
| 规划、控制、轮廓、速度、碰撞区域 | [nav2_params.yaml](src/mynav/config/nav2_params.yaml) |
| 探索周期、目标评分、进展超时 | [explore_params.yaml](src/mynav/config/explore_params.yaml) |
