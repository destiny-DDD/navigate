...
def generate_launch_description():  # 定义生成整个 launch 描述的函数。
    ...
    use_foxglove = LaunchConfiguration("use_foxglove")  # 读取是否启动 Foxglove Bridge 的启动参数。
    ...
    declare_use_foxglove = DeclareLaunchArgument(  # 声明是否启动 Foxglove Bridge 的参数。
        "use_foxglove",  # 启动参数的名字。
        default_value="true",  # 默认启动 Foxglove Bridge。
        description="Start Foxglove Bridge",  # 这个参数在命令行帮助中的说明。
    )  # 结束 use_foxglove 参数声明。
    ...
    foxglove_bridge = Node(  # 创建 Foxglove Bridge 节点，用于连接 Foxglove Studio。
        package="foxglove_bridge",  # 指定节点所属的软件包。
        executable="foxglove_bridge",  # 指定要运行的可执行程序。
        name="foxglove_bridge",  # 指定节点在 ROS 图中的名字。
        output="screen",  # 将节点日志输出到当前终端。
        condition=IfCondition(use_foxglove),  # 只有 use_foxglove 为 true 时才启动此节点。
        parameters=[  # 开始设置节点参数。
            {  # 使用字典传入多个参数。
                "port": 8765,  # 设置 Foxglove WebSocket 服务使用的端口号。
                "use_sim_time": use_sim_time,  # 设置 Foxglove 是否使用仿真时间。
            }  # 结束节点参数字典。
        ],  # 结束参数列表。
    )  # 结束 Foxglove Bridge 节点定义。

    ld = LaunchDescription()  # 创建一个空的 launch 描述对象。
    ld.add_action(declare_use_gui)  # 将 use_gui 参数声明加入 launch 描述。
    ld.add_action(declare_use_foxglove)  # 将 use_foxglove 参数声明加入 launch 描述。
    ...
    ld.add_action(foxglove_bridge)  # 将 Foxglove Bridge 节点加入 launch 描述。
